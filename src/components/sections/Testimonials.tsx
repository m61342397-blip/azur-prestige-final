        ...
        {loaded && avis.length === 0 ? (
          ...
          <>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-white/[0.04]">
              {avis.map((review, i) => (
                <div key={review.id} ref={el => { if (el) cardsRef.current[i] = el; }}
                  className="bg-transparent p-8 flex flex-col gap-6 hover:bg-white/[0.015] transition-colors duration-500"
                  style={{ opacity: 1 }}>
                  <div className="flex gap-1">
                    {[...Array(review.note)].map((_, j) => (
                      <div key={j} className="w-1.5 h-1.5 rounded-full bg-[#D4AF37]" />
                    ))}
                  </div>
                  <p className="text-[#8A8A8A] text-base font-light leading-relaxed flex-1 italic">&ldquo;{review.commentaire}&rdquo;</p>
                  <div>
                    <div className="text-white text-sm font-light">{review.nom}</div>
                    <div className="text-[#A1A1AA] text-[11px] font-light tracking-wide mt-1">{t("verifiedClient")}</div>
                  </div>
                </div>
              ))}
            </div>
